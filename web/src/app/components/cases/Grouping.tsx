...
export default function Grouping({ pdfName }: { pdfName: string }) {
  ...
  async function handleOrganizePdf() {
    setLoading(true);
    try {
      console.log(documentOfInterest, dateOfinterest, yearOfInterest);
      const response = await axios.post(
        `${config.backendUrl}/api/v1/pdf/organize-pdf/${pdfName}`,
        {
          types_of_documents: documentOfInterest,
          dates_of_interests: dateOfinterest,
          years_of_interests: yearOfInterest,
        },
      );
      if (response.status === 200) {
      ...
      <div className="bg-white p-6 w-full gap-4 flex flex-col">
        <div className="flex flex-col gap-4">
          <div className="text-md font-semibold">
            Generate grouped PDF documents that are ordered per year
          </div>
